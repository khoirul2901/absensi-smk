...
function getHariIniIndo() {
  const dayStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "EEEE");
  const mapDays = {
    "Sunday": "Minggu",
    "Monday": "Senin",
    "Tuesday": "Selasa",
...
function simpanBulkAbsenManual(ids, kategori, mode, tanggal, status, keterangan) {
  try {
    ...
    ids.forEach(idTarget => {
        ...
        if (finalStatus === "Hadir (Auto)") {
          finalStatus = hitungStatusHadir(idTarget, kategori, jamSaatIni, ss);
        }
        ...
        if (rowIndex) {
            if (mode === 'Masuk') {
              targetLaporanSheet.getRange(rowIndex, kategori==='Siswa' ? 6 : 5, 1, 2).setValues([[jamSaatIni, finalStatus]]);
              targetLaporanSheet.getRange(rowIndex, kategori==='Siswa' ? 10: 9).setValue(keterangan);
            } else {
              targetLaporanSheet.getRange(rowIndex, kategori==='Siswa' ? 8 : 7, 1, 2).setValues([[jamSaatIni, finalStatus]]);
              targetLaporanSheet.getRange(rowIndex, kategori==='Siswa' ? 10: 9).setValue(keterangan);
            }
        } else {
            ...
            if (md) {
                ...
                let newRow = kategori === 'Siswa' ? 
                   (mode === 'Masuk' ? [idLog, tanggal, idTarget, namaTarget, kelasAtauGuru, jamSaatIni, finalStatus, "-", "-", keterangan] 
                                     : [idLog, tanggal, idTarget, namaTarget, kelasAtauGuru, "-", "-", jamSaatIni, finalStatus, keterangan]) :
                   (mode === 'Masuk' ? [idLog, tanggal, idTarget, namaTarget, jamSaatIni, finalStatus, "-", "-", keterangan] 
                                     : [idLog, tanggal, idTarget, namaTarget, "-", "-", jamSaatIni, finalStatus, keterangan]);
...
function getLiveAbsenHariIni(kategori, tanggal, filterKelas) {
  try {
    ...
    for (let i = 0; i < masterData.length; i++) {
      ...
      if (kategori === 'Siswa') {
         kelasFull = `${mRow.kelas} ${mRow.jurusan}`;
         if(filterKelas && filterKelas !== 'Semua' && kelasFull !== filterKelas) continue;
      }
