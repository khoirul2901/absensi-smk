...
function buatStrukturDatabaseOtomatis() {
  try {
    ...
    const sheetsToCreate = [
      ...
      { name: "pengaturan_jam", headers: ["parameter", "nilai", "keterangan"], data: [["jam_masuk_mulai", "06:00", "Buka scan masuk"], ["jam_masuk_batas", "07:15", "Batas telat masuk"], ["jam_pulang_mulai", "15:30", "Buka scan pulang"], ["api_token", "sias_token_smkalhikam", "Token keamanan API (Simpan rahasia)"]] },
...
function getApiToken() {
  // 1. Coba ambil dari Script Properties (Sangat Aman)
  try {
    const propToken = PropertiesService.getScriptProperties().getProperty("API_TOKEN");
    if (propToken && propToken.trim() !== "") return propToken.trim();
  } catch(e) {}
  
  // 2. Coba ambil dari sheet pengaturan_jam (Mudah Dilihat & Diedit User)
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = ss.getSheetByName("pengaturan_jam");
    if (!sheet) {
    ...
    // Auto-Healing: Jika baris api_token belum ada, buat otomatis
    const defaultToken = "sias_token_smkalhikam";
    sheet.appendRow(["api_token", defaultToken, "Token keamanan API (Simpan rahasia)"]);
    return defaultToken;
  } catch(e) {
    // Fallback jika spreadsheet belum diinisialisasi
