...
function doPost(e) {
  try {
    ...
    // Validasi Token Keamanan API
    const serverToken = getApiToken();
    if (serverToken && serverToken !== "") {
      if (token !== serverToken) {
        return ContentService.createTextOutput(JSON.stringify({
          success: false,
          message: "Akses Ditolak: Token Keamanan API tidak cocok atau tidak sah. Periksa pengaturan aplikasi Anda."
        })).setMimeType(ContentService.MimeType.JSON);
      }
    }
