...
export const IdCard: React.FC<IdCardProps> = ({ item, kategori }) => {
  ...
  const schoolName = localStorage.getItem(getStorageKey('cardSchoolName')) || 'SMK AL-HIKAM';
  const schoolAddress = localStorage.getItem(getStorageKey('cardSchoolAddress')) || 'Sendang Agung, Lampung Tengah, Lampung';
  const principalName = localStorage.getItem(getStorageKey('cardPrincipalName')) || 'Ahmad Zaenuri, S.Pd';
